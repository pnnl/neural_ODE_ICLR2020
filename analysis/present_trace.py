...
os.chdir('../')
from system_id_nlin_con import make_dataset_con, Building_DAE, SSM_black_con
...
def plot_trajectories(traj1, traj2, traj3, traj4, labels, figname):
    ...
    for row, (t1, t2, t3, t4, label) in enumerate(zip(traj1, traj2, traj3, traj4, labels)):
        ...
        ax[row].plot(t2, '--', label='cODE')
        ax[row].plot(t3, '-.', label='ODE')
        ax[row].plot(t4, ':', label='RNN')
...
from system_id_nlin import make_dataset, RNN, SSM_black, Building_DAE
...
rnn = RNN(nx, n_m, n_dT, nu, nd, n_hidden, bias=False, device='cpu')
...
plot_trajectories([np.concatenate([xtrue[:, k], devxtrue[:, k], testxtrue[:, k]])
             ...
             *trajs,
               ['$x_1$', '$x_2$', '$x_3$', '$x_4$'], 'present_trace.png')
